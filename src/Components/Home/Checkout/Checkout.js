    ...
    try {
      ...
      const { data } = await createOrder({
        variables: {
          ...
          billingAddress: addressForShipping,
          status: "pending",
        },
      });

      // if (data && data?.createOrder?.paymentMethod === "COD") {
      //   navigate("/order");
      // }

      if (
        data?.createOrder?.user &&
        data?.createOrder?.paymentMethod === "ONLINE"
      ) {
        const { billingAddress, user } = data.createOrder;
        ...
        if (data?.createOrder) {
          ...
          const paymentGatwayFormData = {
            ...
            currency: "INR",
            amount: parseFloat(grandSum),
            redirect_url: process.env.REACT_APP_PAYMENT_RESPONSE_URL,
            cancel_url: process.env.REACT_APP_PAYMENT_RESPONSE_URL,
            language: "EN",
            billing_name: customerInfo.customerName,
            billing_address: `${customerInfo.addressLine1} ${customerInfo.addressLine2}`,
            billing_city: customerInfo.city,
            billing_state: customerInfo.state,
            billing_zip: customerInfo.postalCode,
            billing_country: "India",
            billing_tel: customerInfo.mobileNo,
            billing_email: customerInfo.email,
            delivery_name: customerInfo.customerName,
            delivery_address: `${customerInfo.addressLine1} ${customerInfo.addressLine2}`,
            delivery_city: customerInfo.city,
            delivery_state: customerInfo.state,
            delivery_zip: customerInfo.postalCode,
            delivery_country: "India",
            delivery_tel: customerInfo.mobileNo,
          };
